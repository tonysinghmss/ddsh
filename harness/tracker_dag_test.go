...
func (c *trackerTestComponent) OnWakeUp(ctx *SpatioTemporalContext) {
	...
	if c.wakeOrder != nil {
		*c.wakeOrder = append(*c.wakeOrder, c.name)
	}
	c.mu.Unlock()
	if c.tracker != nil {
...
func (c *trackerTestComponent) OnSleep() {
	...
	if c.sleepOrder != nil {
		*c.sleepOrder = append(*c.sleepOrder, c.name)
	}
	c.mu.Unlock()
	if c.tracker != nil {
...
func TestDependencyTrackerChainActivationAndDeactivation(t *testing.T) {
	...
	a.sleepOrder = &sleepOrder
	b.sleepOrder = &sleepOrder
	c.sleepOrder = &sleepOrder
	...
	if got := fmt.Sprint(sleepOrder); got != "[C B A]" {
		t.Fatalf("sleep order=%s", got)
	}
}
...
func TestDependencyTrackerDiamondTopologicalOrder(t *testing.T) {
	...
	for _, component := range []*trackerTestComponent{a, b, c, d} {
...
func TestDependencyTrackerCycleRejectionIsAtomic(t *testing.T) {
	...
	if _, ok := dt.Graph().Node("A"); ok {
		t.Fatal("failed cycle registration partially installed provider A")
	}
